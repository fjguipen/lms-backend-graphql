...
(async () => {
  ...
  try {
    ...
    // Lessons
    await LessonModel.query().delete();
    ...
    // Contents
    await QuestionModel.query().delete();
    await ContentModel.query().delete();
    ...
    await UserModel.query()
      ...
      .then(async (result) => {
        ...
        await db().raw(
          `SELECT setval('completed_lessons_id_seq', ${
            result.filter((u: any) => u.completed_lessons).length
          })`
        );
    ...
    // Evaluations
    await EvaluationModel.query().delete();
    await EvaluationModel.query()
      .insertGraph(evaluations)
      .then(async (result) => {
        const insertedQuestions = result.flatMap((r: any) => r.answers || [])
          .length;
