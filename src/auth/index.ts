...
import { ResolversContext, Session, FuncType, HighOrderReturnType, ResolverFuncType } from "../types";
import { UserModel } from "../entities/models";
import { ApolloError } from "apollo-server-core";

export async function logIn (_, { input }: MutationLoginArgs, ctx: ResolversContext): Promise<CurrentUser> {
...
export async function authenticatedSession (req: Express.Request): Promise<Session> {
  if(req.session.user){
    // Check user exists
    const user = await UserModel.query()
 ...
 * @param silent Turns off raising errors. Just remove requested data if conditions doesn't meet. Default: false
 */
export function authorize<T extends ResolverFuncType>(fn: T, allowedRoles: string[], silent=false): HighOrderReturnType<T> {
  return (async (...args: Parameters<ResolverFuncType>) => {
    ...
    if (!ctx.session || !ctx.session.user || !ctx.session.user.id){
      ...
      const { rol: userRoles} = await UserModel.query()
        .findById(ctx.session.user.id)
      
      // Check if userRoles includes any of the allowed roles
      if (!userRoles || userRoles.length === 0 || !allowedRoles.some( rol => userRoles.includes(rol) || rol === '*')){
        if (silent){
          return null
        } else {
